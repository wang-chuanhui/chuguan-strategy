...
import { ClockCardConfig, resolveTimeZone } from "../types/homeassistant/panels/lovelace/cards/types";
...
@customElement("chuguan-clock-card-analog")
export class ChuguanClockCardAnalog extends LitElement {
  ...
  render() {
    ...
    const isNumbers = this.config?.face_style?.startsWith("numbers") ?? false;
    const isRoman = this.config?.face_style?.startsWith("roman") ?? false;
    const isUpright = this.config?.face_style?.endsWith("upright") ?? false;
